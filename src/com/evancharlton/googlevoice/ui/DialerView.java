 ...
 */
public class DialerView extends Activity implements View.OnClickListener, View.OnLongClickListener, View.OnKeyListener, TextWatcher {
	...
	public void afterTextChanged(Editable input) {
		...
		if (mDigits.length() != 0) {
			...
			mDigits.setCompoundDrawablesWithIntrinsicBounds(getResources().getDrawable(R.drawable.ic_dial_number), null, null, null);
		} else {
	...
	protected void onCreate(Bundle icicle) {
		...
		if (icicle != null) {
			mDigits.setText(icicle.getString("digits"));
		}
	}
	...
	private boolean resolveIntent() {
		...
		if (isChild()) {
			...
			ignoreState = intent.getBooleanExtra(DialerView.EXTRA_IGNORE_STATE, false);
		} else {
			intent = getIntent();
		}
			...
			if (uri != null) {
				if ("tel".equals(uri.getScheme())) {
					...
					if (People.CONTENT_ITEM_TYPE.equals(type) || Phones.CONTENT_ITEM_TYPE.equals(type)) {
						...
						Cursor c = getContentResolver().query(intent.getData(), new String[] {
	...
	protected void onResume() {
		...
		if (parent != null && parent instanceof GoogleVoice) {
			// TODO: Add support for this
			// Uri dialUri = ((GoogleVoice) parent).getAndClearDialUri();
			// if (dialUri != null) {
			// resolveIntent();
			// }
		}
		...
		switch (keyCode) {
			case KeyEvent.KEYCODE_CALL: {
				if (mIsAddCallMode && (TextUtils.isEmpty(mDigits.getText().toString()))) {
					...
					// phone
					// number entered is empty, we just close the dialer to
					// expose
	...
	void callVoicemail() {
		...
		Intent intent = new Intent(Intent.ACTION_CALL, Uri.fromParts("tel", prefs.getString(PreferencesProvider.GV_NUMBER, ""), null));
		intent.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
		startActivity(intent);
	...
	void placeCall() {
		...
		if (isChild()) {
			((GoogleVoice) getParent()).call(mDigits.getText().toString());
			mDigits.getText().clear();
		}
	}
	...
	public void onSaveInstanceState(Bundle outState) {
		...
		outState.putString("digits", mDigits.getText().toString());
	}
}